...
class NeuralNet(torch.nn.Module):
    def __init__(self):
        ...
        self.optimizer = torch.optim.SGD(self.parameters(), lr=.01,momentum=0.5,weight_decay=0.0)
        self.epochs = 300
...
class MVNeuralNet(NeuralNet):
    ...
    def fit(self,x,y):
        ...
        self.apply(self.weight_reset)
        x = torch.from_numpy(x)
        y = torch.from_numpy(y).reshape(len(y),2)
        #print(y)
        for t in range(self.epochs):
            # Forward pass: Compute predicted y by passing x to the model
            y_pred = self(x)
...
class MVNeuralNet1(MVNeuralNet):
    def __init__(self,k):
        super(MVNeuralNet, self).__init__()
        ...
        self.layer_5 = torch.nn.Linear(10,2)       

        self.criterion = torch.nn.MSELoss()
        self.optimizer = torch.optim.SGD(self.parameters(), lr=.01,momentum=0.5,weight_decay=0)
        self.epochs = 100
...
class NeuralNet1(NeuralNet):
    def __init__(self,k):
        ...
        self.layer_1.add_module("D1", nn.Dropout(0.2))
        ...
        self.layer_2.add_module("D2", nn.Dropout(0.2))
        # self.layer_3 = nn.Sequential()
        # self.layer_3.add_module("L3", nn.Linear(10,10))
        ...
        self.criterion = torch.nn.MSELoss()
        self.optimizer = torch.optim.SGD(self.parameters(), lr=.05,momentum=0.3,weight_decay=0)
        self.epochs = 1000
